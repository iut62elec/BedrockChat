...
USER_ICON = "pics/user-icon.png"
AI_ICON = "pics/ai-icon.png"
...
def write_top_bar():
    ...
    with col2:
        ...
        header = f"Bedrockchat powered by Amazon Kendra & AWS Bedrock!"

        st.write(f"<h3 class='main-header'>{header}</h3>", unsafe_allow_html=True)
    with col3:
        clear = st.button("Clear Chat")
        logout = st.button("Log Out") # add log out button
        if logout:
            handle_logout()
...
st.markdown('---')
input = st.text_input("You are talking to BedrockChat, ask any question.", key="input", on_change=handle_input)
